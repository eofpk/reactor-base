...
static void handleReadEvent(void* instance)
{
   ...
   if(0 < receiveResult) {
      ...
      printf("%s, %d == Client: Diagnostics received - %s\n", __FILE__, __LINE__, diagnosticMessage);
   }
   else {
...
static Handle acceptClientConnection(int serverHandle)
{
   ...
   printf("%s, %d == Client: New connection created on IP-address %X\n", __FILE__, __LINE__, ntohl(clientAddress.sin_addr.s_addr));
